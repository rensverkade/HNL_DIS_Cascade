...
hf = h5.File('Struct_func_dark.hdf5', 'r')
...
F2_x_yZ = np.array( hf.get('F2_x_yZ2'))
F2_x_Z =  np.array(hf.get('F2_x_Z2'))
F3_x_y = np.array( hf.get('F3_x_y'))
F3_x_yZ =  np.array(hf.get('F3_x_yZ2'))
F3_x_Z = np.array( hf.get('F3_x_Z2'))

F2_Q_y =  np.array(hf.get('F2_Q_y'))
F2_Q_yZ = np.array( hf.get('F2_Q_yZ2'))
F2_Q_Z =  np.array(hf.get('F2_Q_Z2'))
F3_Q_y = np.array( hf.get('F3_Q_y'))
F3_Q_yZ =  np.array(hf.get('F3_Q_yZ2'))
F3_Q_Z = np.array( hf.get('F3_Q_Z2'))
...
for i in range(l): #l 0-8        
    ...
    if  i==5:     
        ...
        axs[1,1].plot(xs_x,F2_x_yZ[i,:],color=colours[i], label = r'$F^{\gamma Z^\prime}_2$', ls='dashed')
        axs[1,1].plot(xs_x,F2_x_Z[i,:],color=colours[i], label = r'$F_2^{Z^\prime}$', ls= 'dotted')
...
axs[0,1].set_ylim(-0.00001,0.001)
axs[1,0].set_ylim(-0.1*10**(-7),1*10**(-7))
#axs[1,1].set_ylim(-0.00001,0.5)
plt.xscale('log')
#plt.yscale('log')
#plt.ylim(0,1.5)
fig.tight_layout()
...
plt.savefig('Struct_figs_Dark_SSM/F2_p_x.pdf', dpi=800,bbox_inches = 'tight')
if close == True:
    plt.close()
else:
    plt.show()
...
for i in range(l): #l 0-8        
    ...
    if  i==5:     
        ...
        axs[1,1].plot(qs_Q,F2_Q_yZ[i,:],color=colours[i], label = r'$F^{\gamma Z^\prime}_2$', ls='dashed')
        axs[1,1].plot(qs_Q,F2_Q_Z[i,:],color=colours[i], label = r'$F_2^{Z^\prime}$', ls= 'dotted')
...
fig.legend(loc='center right')
plt.subplots_adjust(hspace=0.1,right=0.75)
plt.savefig('Struct_figs_Dark_SSM/F2_p_Q2.pdf', dpi=800,bbox_inches = 'tight')
if close == True:
    plt.close()
else:
    plt.show()
