...
def struct_PDF(x,Q2):
    ...
    F2_Z = np.sum((g_V**2+g_A**2) *(xf[f_half:]+xf[:f_half]))
    F2_yZ = np.sum(2*e_q*g_V *(xf[f_half:]+xf[:f_half]))

    xF3_Z = np.sum(2*g_V*g_A *(xf[f_half:]-xf[:f_half]))
    xF3_yZ = np.sum(2*e_q*g_A *(xf[f_half:]-xf[:f_half]))
    ...
    return (F2_y,F2_Z,F2_yZ,xF3_y,xF3_Z,xF3_yZ)   # (x2F1,
...
F1_x_yZ = np.zeros([len(qs_x), len(xs_x)])
F2_x_yZ = np.zeros([len(qs_x), len(xs_x)])
F3_x_yZ = np.zeros([len(qs_x), len(xs_x)])
F1_Q_yZ = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)
F2_Q_yZ = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)
F3_Q_yZ = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)

F1_x_Z = np.zeros([len(qs_x), len(xs_x)])
F2_x_Z = np.zeros([len(qs_x), len(xs_x)])
F3_x_Z = np.zeros([len(qs_x), len(xs_x)])
F1_Q_Z = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)
F2_Q_Z = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)
F3_Q_Z = np.zeros([len(xs_Q), len(qs_Q)]) #(8,80)

for i in range(l): #l 0-8
    for j in range(k): #0-80   p.xfxQ2 takes Q^2 as arg
        F2_Q_y[i,j], F2_Q_yZ[i,j],F2_Q_Z[i,j],F3_Q_y[i,j],F3_Q_yZ[i,j],F3_Q_Z[i,j] = struct_PDF(xs_Q[i], qs_Q[j])#taking Q^2 as argument
        F2_x_y[i,j] ,F2_x_yZ[i,j] ,F2_x_Z[i,j] ,F3_x_y[i,j],F3_x_yZ[i,j],F3_x_Z[i,j]  = struct_PDF(xs_x[j], qs_x[i])#taking Q^2 as argument
        


hf = h5.File('Struct_func.hdf5', 'w')
...
hf.create_dataset('F2_x_yZ', data=F2_x_yZ)
hf.create_dataset('F2_x_Z', data=F2_x_Z)

hf.create_dataset('F3_x_y', data=F3_x_y)
hf.create_dataset('F3_x_yZ', data=F3_x_yZ)
hf.create_dataset('F3_x_Z', data=F3_x_Z)

hf.create_dataset('F2_Q_y', data=F2_Q_y)
hf.create_dataset('F2_Q_yZ', data=F2_Q_yZ)
hf.create_dataset('F2_Q_Z', data=F2_Q_Z)
hf.create_dataset('F3_Q_y', data=F3_Q_y)
hf.create_dataset('F3_Q_yZ', data=F3_Q_yZ)
hf.create_dataset('F3_Q_Z', data=F3_Q_Z)
