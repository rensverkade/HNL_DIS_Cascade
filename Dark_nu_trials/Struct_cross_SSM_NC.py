...
def struct_PDF(x,Q2):
    ...
    F2_Z2 = np.sum((g_Z1_V**2+g_Z1_A**2) *(xf[f_half:]+xf[:f_half]))
    #F2_yZ2 = np.sum(2*e_q*g_Z2_V *(xf[f_half:]+xf[:f_half]))
    xF3_Z2 = np.sum(2*g_Z1_V*g_Z1_A *(xf[f_half:]-xf[:f_half]))
...
def ds_v(x,y,Vert,E,anti): #M is nucleon mass
    ...
    #anti =1
    couple = g_Z1_V_H*g_Z1_V_D
    #(c_B+s_B*s_W*t_X)*g_g*s_B*c_W*t_X*0.5/c_W #C_NC * C_EM
    ds = (Vert*couple)**2  *mp*E/(np.pi*(M_Z1**2+Q2)**2) *(ds1+anti*ds2) 
    return ds
