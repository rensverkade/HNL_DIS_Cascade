...
def struct_PDF_Wm(x,Q2):
    ...
    FL_W_m= (R_1+1)*F2_W_m #WHERE am is supposeed to find disnshit
    
    x2F1_W_m = F2_W_m*(1+4*mp**2 * x**2 /Q2)/(1+FL_W_m)
...
def struct_PDF_Wp(x,Q2):
    ...
    FL_W_p= (R_1+1)*F2_W_p #WHERE am is supposeed to find disnshit
    x2F1_W_p = F2_W_p*(1+4*mp**2 * x**2 /Q2)/(1+FL_W_p)
